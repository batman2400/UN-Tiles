...
5. Vision model: `gemini-3.5-flash-lite` (structured JSON). Ultra-fast sub-second response times, high quota availability, image + text in, structured outputs supported. Fallback order: `gemini-flash-lite-latest`, then `gemini-3.6-flash`. Do not use image-generation models (`*-flash-image`). Do not set `thinkingBudget: 0` as modern Gemini 3.x models reject it with HTTP 400.
